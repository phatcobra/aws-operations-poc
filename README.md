...
→ sanitized public operations dashboard
...
| GitHub Pages | Public operations dashboard | Sanitized health, heartbeat, recent runs, recovery, proof, and walkthrough mode |
...
The repository also contains a static, read-only dashboard for people who do not have AWS Console access. It keeps the operational signals visible instead of hiding them behind a marketing-style explanation.

The dashboard shows:

- overall health and the last scheduled run;
- heartbeat / alarm meaning and the supporting operating checks;
- recent run outcomes with trigger, attempts, and latency;
- a compact outcome timeline and deployment-proof summary;
- an **Explain mode** walkthrough that moves through the dashboard in the same order a presenter would use it.
...
site/                 public operations dashboard and fallback status contract
