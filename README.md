...
→ interactive read-only public experience
...
| GitHub Pages | Built to recover | Interactive simulation, recorded attempt evidence, and optional engineering detail |
...
## Built to recover: the public experience

The static public site is an interactive explanation for both nontechnical visitors and engineers. Its central experience lets visitors choose normal, transient, or permanent failure and follow a simulated request with play/pause, step controls, restart, and a keyboard-operable scrubber. Presentation mode shows talking points and uses manual advance. Selecting a component reveals its implementation and an exact repository source link.

Simulation and real evidence are deliberately separate:

- The opening status describes the last published check, never an unqualified live health claim.
- The interactive story is labeled simulation; no control invokes AWS.
- The evidence viewer filters hourly checks and deployment tests, with selectable recorded attempt sequences.
- The latest hourly result remains in the bounded 12-run sample even after a burst of deployment tests.
- The deployment summary is validated from the exact CD run's downloaded evidence artifact. A URL alone is not proof.
- Engineering disclosures cover retries, OIDC, persistence, monitoring limitations, and the public data boundary.
- Missing, invalid, future-dated, or older-than-two-hour snapshots fail closed to unknown. Historical evidence remains readable.
- Per-attempt latency is explicitly not total run time, and deliberate tests are not labeled production incidents.
...
The public-site workflow publishes the `site/` directory to GitHub Pages after successful main-branch CD, checking out that exact SHA and reading its proof artifact using repository-scoped Actions read access. Manual dispatch is available for a status refresh; without an attached artifact it publishes proof as unknown. GitHub Pages uses **Source: GitHub Actions**. There is no continuous public status polling or new AWS endpoint. Reduced-motion visitors get manual story stepping.

Public UI regression checks: `node --test tests/site.test.mjs` (Node 22+). CI, CD, and the public publish workflow run them. Python publisher tests verify sanitization, artifact validation, and conservative health reporting.
...
site/                 interactive public experience and sanitized status contract
